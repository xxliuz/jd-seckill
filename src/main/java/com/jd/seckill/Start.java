...
public class Start {
    ...
    public static void judgePruchase() throws IOException, ParseException, InterruptedException {
        ...
        if (shopDetail.get("yuyueInfo") != null) {
            ...
            Long serverTime;
            while (true) {
                jdTime = JSONObject.parseObject(HttpUrlConnectionUtil.get(headers, "https://api.m.jd.com/client.action?functionId=queryMaterialProducts&client=wh5"));
                serverTime = Long.valueOf(jdTime.get("currentTime2").toString());
